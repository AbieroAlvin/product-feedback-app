...
const Sidebar = ({ allFeedbacks }: { allFeedbacks: productType[] }) => {
  ...
  return (
    <aside className="max-w-[25.5rem] space-y-[2.4rem] laptop:grid laptop:h-[18rem] laptop:w-full laptop:max-w-full laptop:grid-cols-[1fr_2fr] laptop:gap-4 laptop:space-y-0 tablet:relative tablet:h-[7.5rem] tablet:grid-cols-1">
      ...
      <div
        className={`z-[11] space-y-[2.4rem] laptop:grid laptop:grid-cols-2 laptop:gap-4 laptop:space-y-0 tablet:fixed tablet:right-0 tablet:top-[7.5rem] tablet:flex tablet:h-full tablet:w-[75%] tablet:flex-col tablet:bg-[#f7f8fd] ${
          isNavOpen ? "tablet:block" : "tablet:hidden"
        }`}
      >
