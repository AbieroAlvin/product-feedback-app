...
const FeedbackDetail = () => {
  ...
  const handlePostComment = (e: SyntheticEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  return (
    <section className="relative mx-auto flex min-h-[100dvh] w-full max-w-[82rem] flex-col justify-center gap-[2.4rem] py-8 tablet:grid-cols-[auto_1fr_auto] tablet:px-8">
      ...
      <div className="group grid cursor-pointer grid-cols-[auto_1fr_auto] gap-16 rounded-[1rem] bg-white px-[3.2rem] py-[2.8rem] tablet:px-[2.4rem]">
        <button
          className={`flex flex-col items-center self-start rounded-[1rem] p-4 transition-all duration-300 tablet:col-start-1 tablet:row-start-2 tablet:flex-row tablet:gap-3  ${
            isUpvoted
              ? "bg-[#4661e6] text-white"
              : "bg-[#f2f4fe] text-[#4661e6] hover:bg-[#cfd7ff]"
          }`}
          onClick={() => {
        ...
        >
          ...
          <p
            className={`text-[1.3rem] font-bold tracking-[-0.0181rem] ${
              isUpvoted ? "text-white" : "text-[#3a4374]"
            }`}
          >
            {getFeedback?.upvotes}
          </p>
      ...
      <div
        className={`space-y-[3.2rem] divide-y divide-[#8c92b3] divide-opacity-25 bg-white px-[3.2rem] pb-[4.8rem] pt-[2.4rem] tablet:px-[2.4rem] tablet:pt-0 ${
          getFeedback?.comments.length === 0 ? "hidden" : ""
        }`}
      >
      ...
      >
        ...
        <textarea
          ...
          className={`w-full resize-none rounded-[0.5rem] border border-solid border-transparent bg-[#f7f8fd] px-[2.4rem] py-[1.2rem] text-[1.5rem]  text-[#3a4374] ${
            commentText.length < 3
              ? "border-[#d73737] focus:border-[#d73737] focus:outline-[#d73737]"
              : "focus:border-[#4661e6] focus:outline-[#4661e6]"
          }`}
        />
