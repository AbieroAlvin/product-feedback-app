...
const RoadmapStatusBox = ({ data }: { data: productType }) => {
  ...
  return (
    <Link
      ...
      className={`group flex min-h-[28rem] flex-col rounded-[0.5rem] border-t-[6px] border-solid bg-white p-[3.2rem] laptop:px-8 tablet:min-h-full ${
        data.status === "planned"
          ? "border-[#f49f85]"
    ...
    >
      ...
      <div className="flex items-center justify-between pt-[1.6rem]">
        <button
          className={`flex items-center gap-3 self-start rounded-[1rem] p-4 transition-all duration-300  disabled:bg-[#ccc] ${
            isUpvoted
